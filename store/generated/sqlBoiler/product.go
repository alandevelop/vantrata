...
var ProductWhere = struct {
	...
	Capacity:   whereHelperint{field: "\"product\".\"capacity\""},
	Price:      whereHelperint{field: "\"product\".\"price\""},
	CurrencyID: whereHelperstring{field: "\"product\".\"currency_id\""},
}
...
type productR struct {
	...
	Availabilities AvailabilitySlice `boil:"Availabilities" json:"Availabilities" toml:"Availabilities" yaml:"Availabilities"`
...
func (r *productR) GetCurrency() *Currency {
	if r == nil {
		return nil
	}
	return r.Currency
...
func (r *productR) GetAvailabilities() AvailabilitySlice {
	if r == nil {
		return nil
	}
...
func (r *productR) GetBookings() BookingSlice {
	if r == nil {
		return nil
	}
	return r.Bookings
...
var (
	productAllColumns            = []string{"id", "name", "capacity", "price", "currency_id"}
	productColumnsWithoutDefault = []string{"id", "name", "capacity", "price", "currency_id"}
...
// Availabilities retrieves all the availability's Availabilities with an executor.
func (o *Product) Availabilities(mods ...qm.QueryMod) availabilityQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"availability\".\"product_id\"=?", o.ID),
	)
...
func (o *Product) Bookings(mods ...qm.QueryMod) bookingQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"booking\".\"product_id\"=?", o.ID),
	)
...
// LoadAvailabilities allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (productL) LoadAvailabilities(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProduct interface{}, mods queries.Applicator) error {
	var slice []*Product
	var object *Product
	...
	query := NewQuery(
		...
		qm.WhereIn(`availability.product_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on availability")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.Availabilities = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ProductID {
				local.R.Availabilities = append(local.R.Availabilities, foreign)
				if foreign.R == nil {
					foreign.R = &availabilityR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (productL) LoadBookings(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProduct interface{}, mods queries.Applicator) error {
	var slice []*Product
	var object *Product
	...
	query := NewQuery(
		qm.From(`booking`),
		qm.WhereIn(`booking.product_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddAvailabilities adds the given related objects to the existing relationships
// of the product, optionally inserting them as new records.
// Appends related to o.R.Availabilities.
// Sets related.R.Product appropriately.
func (o *Product) AddAvailabilities(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Availability) error {
	var err error
	...
	if o.R == nil {
		o.R = &productR{
			Availabilities: related,
		}
	} else {
		o.R.Availabilities = append(o.R.Availabilities, related...)
	}
...
// Sets related.R.Product appropriately.
func (o *Product) AddBookings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Booking) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ProductID = o.ID
